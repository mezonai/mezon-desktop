...
class VideoTrackSource {
  class InternalSource : public webrtc::AdaptedVideoTrackSource {
   public:
    ...
    std::shared_ptr<mezon_rtc::EncodedRateControlState> rate_control_state()
        const {
   ...
   private:
    ...
    std::shared_ptr<mezon_rtc::EncodedRateControlState> rate_control_state_ =
        std::make_shared<mezon_rtc::EncodedRateControlState>();
    bool is_screencast_;
  };
