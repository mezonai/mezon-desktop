...
mod internal {
    ...
    use crate::core_media::{
        ...
        AVMediaTypeTimecode, AVMediaTypeVideo, CGPoint, CMSampleBufferGetImageBuffer, CMTimeMake,
    ...
    use core_media_sys::{
        ...
        CMSampleBufferRef, CMTime, CMVideoDimensions,
    };
    use core_video_sys::{
    ...
    impl AVCaptureDevice {
        ...
        pub fn lock(&mut self) -> Result<(), NokhwaError> {
            ...
            // AVFoundation expects NSError **, not a null NSError *.
            let mut error: *mut Object = std::ptr::null_mut();
            let accepted: BOOL = unsafe { msg_send![self.inner, lockForConfiguration: &mut error] };
            if accepted == NO {
                let detail = if error.is_null() {
                    "Lock Rejected".to_string()
                } else {
                    let description: *mut Object = unsafe { msg_send![error, localizedDescription] };
                    nsstr_to_str(description).into_owned()
                };
        ...
        pub fn set_all(&mut self, descriptor: CameraFormat) -> Result<(), NokhwaError> {
            self.lock()?;
            let result = self.set_all_locked(descriptor);
            // Release the lock even when format discovery or selection fails.
            self.unlock();
            result
        }

        fn set_all_locked(&mut self, descriptor: CameraFormat) -> Result<(), NokhwaError> {
            let format_list = try_ns_arr_to_vec::<AVCaptureDeviceFormat, NokhwaError>(unsafe {
                msg_send![self.inner, formats]
            })?;
            // Keep the format and its frame-rate range together. Several native
            // formats can share a resolution but have different pixel formats/rates.
            let selected = format_list.iter().find_map(|format| {
                if format.resolution.width != descriptor.resolution().width() as i32
                    || format.resolution.height != descriptor.resolution().height() as i32
                    || format.fourcc != descriptor.format()
                {
                    return None;
                }
                ns_arr_to_vec::<AVFrameRateRange>(unsafe {
                    msg_send![format.internal, videoSupportedFrameRateRanges]
                })
                .into_iter()
                .find_map(|range| {
                    crate::frame_rate::select_frame_duration(
                        descriptor.frame_rate(),
                        range.min(),
                        range.max(),
                    )
                    .map(|duration| (format.internal, range.inner, duration))
                })
            });
            let Some((selected_format, selected_range, duration)) = selected else {
                return Err(NokhwaError::SetPropertyError {
            ...
            // Use native durations at range boundaries (e.g. 29.97 FPS), and
            // the requested duration for rates inside the supported range.
            let frame_duration: CMTime = match duration {
                crate::frame_rate::FrameDuration::Minimum => unsafe {
                    msg_send![selected_range, minFrameDuration]
                },
                crate::frame_rate::FrameDuration::Maximum => unsafe {
                    msg_send![selected_range, maxFrameDuration]
                },
                crate::frame_rate::FrameDuration::Exact(fps) => unsafe {
                    CMTimeMake(1, fps as i32)
                },
            };
            unsafe {
                let _: () = msg_send![self.inner, setActiveFormat: selected_format];
                let _: () = msg_send![self.inner, setActiveVideoMinFrameDuration: frame_duration];
                let _: () = msg_send![self.inner, setActiveVideoMaxFrameDuration: frame_duration];
            }
            Ok(())
        }
