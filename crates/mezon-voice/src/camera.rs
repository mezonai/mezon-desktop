...
pub fn start_camera(
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            if !request_macos_permission() {
                let _ = ready_tx.send(Err("camera permission denied".into()));
                return;
            }
            ...
            'outer: loop {
                let switch_to = 'capture: loop {
                    ...
                    push_camera_frame(&buffer, &frame_tx, &frame_store, key, &mut preview);
                };
    ...
    (CameraController { stop, switch_tx }, ready_rx, frame_rx)
}

fn push_camera_frame(
    buffer: &nokhwa::Buffer,
    frame_tx: &flume::Sender<I420Frame>,
    ...
    match buffer.source_frame_format() {
        FrameFormat::YUYV => {
            yuyv422_to_i420(buffer.buffer(), w, h, &mut y, &mut u, &mut v, w, cw, cw)
        }
        _ => {
            ...
            let rw = (decoded.width().min(dw) & !1).max(2) as usize;
            let rh = (decoded.height().min(dh) & !1).max(2) as usize;
            rgb_to_i420(decoded.as_raw(), rw, rh, &mut y, &mut u, &mut v, w, cw, cw);
        }
    }

    let (y, u, v) = flip_i420(&y, &u, &v, w, h, cw, ch);

    preview.resize(w * h * 4, 0);
    crate::video::i420_to_bgra_into(preview, &y, &u, &v, w, cw, cw, w, h);
    if let Some(recycled) = frame_store.publish(key, dw, dh, std::mem::take(preview)) {
        *preview = recycled;
    }

    let _ = frame_tx.try_send(I420Frame::tightly_packed(dw, dh, y, u, v));
}
...
fn flip_i420(
    ...
    let mut du = vec![0u8; cw * ch];
    let mut dv = vec![0u8; cw * ch];
    flip_plane(y, w, &mut dy, w, w, h);
    flip_plane(u, cw, &mut du, cw, cw, ch);
    flip_plane(v, cw, &mut dv, cw, cw, ch);
    (dy, du, dv)
}
...
fn push_camera_frame_into_source(
    buffer: &nokhwa::Buffer,
    source: &NativeVideoSource,
    frame_store: &VideoFrameStore,
    key: u64,
